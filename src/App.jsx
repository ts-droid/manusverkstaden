            ...
            {(() => {
              ...
              const friendlyName = (key) => {
                ...
                const nameMap = {
                  ...
                  translate: "Översättning (bas)", translate_en: "Översättning → Engelska",
                  translate_de: "Översättning → Tyska", translate_es: "Översättning → Spanska",
                  translate_ar: "Översättning → Arabiska", final_check: "Slutkontroll",
                ...
                <>
                  <div style={{ display: "flex", gap: 4, marginBottom: 20, flexWrap: "wrap" }}>
                    {categories.map(c => {
                      ...
                      if (!c.special && c.id !== "all" && count === 0) return null;
                      return (
                        <button key={c.id} onClick={() => {
                          setPromptCategory(c.id);
                          if (c.id === "wordlist" && wordList.length === 0) {
                            // Load word list data on first click
                            (async () => {
                              try {
                                const [wlData, rejData] = await Promise.all([
                                  apiClient.getAdminWordList(),
                                ...
                                setWordList(wlData.entries || []);
                                setRejectedPatterns(rejData.patterns || []);
                              } catch (err) { console.error("Word list fetch failed:", err); }
                            })();
                          }
                        }} style={{
                          ...
                          background: promptCategory === c.id ? (c.special ? accent : `${accent}14`) : (c.special ? `${accent}08` : surface),
                          color: promptCategory === c.id ? (c.special ? "#fff" : accent) : (c.special ? accent : muted),
                          transition: "all 0.15s",
                          ...(c.special ? { marginLeft: "auto" } : {}),
                        }}>{c.special ? c.label : `${c.label} (${count})`}</button>
                      );
                    })}
                  </div>
                  ...
                  {/* ─── Prompt list (when not wordlist) ─── */}
                  {promptCategory !== "wordlist" && <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                    {filtered.map(p => {
